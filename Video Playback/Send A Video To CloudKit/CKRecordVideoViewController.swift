//
//  CKRecordVideoViewController.swift
//  SoundAdvice
//
...
class CKRecordVideoViewController: UIViewController, AVCaptureFileOutputRecordingDelegate {
    ...
    // MARK: KEY VALUE OBSERVER PROPERTIES
    ...
    override func viewDidLoad() {
        ...
        // Set up videoRecordingView
        recordVideoView.session = session
        ...
        default:
            setupResult = .notAuthorized // The user has previously denied access.
        }
        ...
        sessionQueue.async {
            ...
            self.configureSessionForVideoRecording()
        }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        sessionQueue.async {
            ...
            case .notAuthorized:
                DispatchQueue.main.async {
                    let changePrivacySetting = "Sound Advice doesn't have permission to use the camera, please change privacy settings"
    ...
    override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        if let videoPreviewLayerConnection = recordVideoView.videoPreviewLayer.connection {
...
//                self.videoURL = previewVideo.videoAsset?.fileURL?.appendingPathExtension("mov") // Maybe???
        ...
        do {
            ...
            if session.canAddInput(videoDeviceInput) {
                ...
                DispatchQueue.main.async {
                    ...
                    self.recordVideoView.videoPreviewLayer.connection?.videoOrientation = initialVideoOrientation
                }
            } else {
    ...
    @IBAction private func changeCamera(_ cameraButton: UIButton) {
        ...
        sessionQueue.async {
            ...
            DispatchQueue.main.async {
                self.chooseCameraButton.isEnabled = true
                self.recordVideoButton.isEnabled = self.movieFileOutput != nil
    ...
    @IBAction private func focusAndExposeTap(_ gestureRecognizer: UITapGestureRecognizer) {
        let devicePoint = recordVideoView.videoPreviewLayer.captureDevicePointConverted(fromLayerPoint: gestureRecognizer.location(in: gestureRecognizer.view))
    ...
    @IBAction private func toggleMovieRecording(_ recordButton: UIButton) {
        ...
        let videoPreviewLayerOrientation = recordVideoView.videoPreviewLayer.connection?.videoOrientation
...
//                print(self.videoURL)
                // MARK: FIXME - TODO: Added last night, trying to coordinate the right URL to use
                let outputURL = self.assignedURL()
                VideoRecordingController.sharedInstance.videoURL = outputURL
...
//                movieFileOutput.startRecording(to: URL(fileURLWithPath: outputFilePath), recordingDelegate: self)
    ...
    func fileOutput(_ output: AVCaptureFileOutput,
        ...
        // MARK: FIXME - TODO: Move cleanup into the completion of save(video) so we can be tidy
        func cleanup() {
        ...
        if success {
            VideoRecordingController.sharedInstance.videoURL = self.videoURL // Assign videoURL to VideoRecordingController singleton
            // Check the authorization status. Needed to import Photos for this part
            PHPhotoLibrary.requestAuthorization { status in
                if status == .authorized {
                    // Save the movie file to the photo library and cleanup.
                    PHPhotoLibrary.shared().performChanges({
                        let options = PHAssetResourceCreationOptions()
                        options.shouldMoveFile = false // This bool lets the asset be used later
...
//            cleanup()
        ...
        DispatchQueue.main.async {
            ...
            if self.videoURL != nil {
                self.programmaticPlayback() // Pop modal for video playback
            }
    ...
    private func addObservers() {
        let keyValueObservation = session.observe(\.isRunning, options: .new) { _, change in
            ...
            DispatchQueue.main.async {
                // Only enable the ability to change camera if the device has more than one camera.
                self.chooseCameraButton.isEnabled = isSessionRunning && self.videoDeviceDiscoverySession.uniqueDevicePositionsCount > 1
                self.recordVideoButton.isEnabled = isSessionRunning && self.movieFileOutput != nil
            }
        }
...
// MARK: EXTENSIONS
extension CKRecordVideoViewController {
    func configureSessionForVideoRecording() {
        sessionQueue.async {
            let movieFileOutput = AVCaptureMovieFileOutput()
